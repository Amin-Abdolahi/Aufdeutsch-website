...
export function Testimonials({
  ...
  // ---------- حالت بارگذاری (اسکلتون کاغذی روی تخته) ----------
  if (loading) {
    return (
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {[1, 2, 3].map((i) => (
            <div
              ...
              style={{
                background:
                  "linear-gradient(180deg, #fefcf6 0%, #faf5e8 100%)",
  ...
  // 📐 تیلت بیشتر (ولی هنوز نامحسوس)
  const tilts = ["-1.6deg", "1.2deg", "-1.1deg", "1.8deg", "-1.4deg", "0.9deg", "-1.3deg"];
  ...
  return (
    <div
      ...
      onMouseEnter={(e) => {
        ...
        const tapeEl = e.currentTarget.querySelector<HTMLElement>("[data-tape]");
        if (tapeEl) {
          const isTranslated = tapeEl.dataset.translated === "true";
          tapeEl.style.transform = isTranslated
            ? `translateX(-50%) rotate(${tapeAngleNum + (index % 2 === 0 ? -3 : 3)}deg)`
    ...
    >
      ...
      <div
        data-tape="true"
        data-translated={tapePosition.includes("translate") ? "true" : "false"}
  ...
  <div className="text-center pt-12 relative z-10">
    ...
    >
      {/* متن دست‌نویس (اختیاری - قابل حذف) */}
      <span
        className="font-bold text-sm tracking-[0.15em] opacity-80 group-hover/more:opacity-100"
        style={{
